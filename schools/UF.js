...
class UF {
  ...
  recourse(x, res) {
    ...
    var url =
      "https://one.uf.edu/apix/soc/schedule/?category=CWSP&term=2188&last-control-number=" +
      x;
    request(url, function(error, response, html) {
      if (!error) {
        ...
        if (x + 50 > 10832) {
          fs.writeFile(
            "public/json/allPossibleCourses.json",
            JSON.stringify(self.courses),
            "utf8",
            function(err) {
              if (err) {
  ...
  getPossibleCourses(res) {
    ...
    if (this.possibleCourses) {
      ...
      fs.readFile("public/json/allPossibleCourses.json", "utf8", function(
        err,
        data
      ) {
        if (!err) {
