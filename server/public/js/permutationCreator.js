	...
	removeOverlapAndFilter(permutation, courses, filters){
		...
		for(var i =0; i< permutation.length; i++){
			...
			for(var j=0; j< permutation[i].length; j++){
				...
				var sectionMeetTimes = courses[permutation[i][j].course].getSectionTimes(permutation[i][j].section);
				...
				{	//looping thru every day
					for(var k =0; k < sectionMeetTimes[b].meetDays.length; k++)
					{
						...
						for(var l = parseInt(convertToNum(sectionMeetTimes[b].meetPeriodBegin), 10);  l < (parseInt(convertToNum(sectionMeetTimes[b].meetPeriodEnd),10)+1); l++){
							...
							//console.log(key);
							if(hit[key] || !this.checkFilter(sectionMeetTimes[b].meetDays[k], l, filters)) {
									//console.log("hit" + key);
									toDelete=true;
							}else{
